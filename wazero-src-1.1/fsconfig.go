...
type FSConfig interface {
	...
	// WithFauxFdProvider creats a filesystem at guestPath that
	// is handled by a FauxFdProvider.
	//
	// If the same `guestPath` was assigned before, this overrides its value,
	// retaining the original precedence. See the documentation of FSConfig for
	// more details on `guestPath`.
	WithFauxFs(p FauxFs, guestPath string) FSConfig
}
...
// WithFauxFdProvider creates a wrapper around the provider and uses
// that as a filesystem.
func (c *fsConfig) WithFauxFs(p FauxFs, guestPath string) FSConfig {
	return c.withMount(newFauxFs(p), guestPath)
}
