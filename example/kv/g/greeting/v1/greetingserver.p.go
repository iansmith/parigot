...
func Launch(ctx context.Context, sid id.ServiceId, impl Greeting) *future.Base[bool] {
	...
	ready.Handle(func (b bool) {
		...
		slog.Error("Unable to start greeting.v1.Greeting, Ready returned false")
...
func Init(require []lib.MustRequireFunc, impl Greeting) (*lib.ServiceMethodMap,*syscallguest.LaunchFuture, context.Context, id.ServiceId){ 
	...
	launchF.Failure(func (err syscall.KernelErr) {
		...
		slog.Error("launch failure on call Greeting","error",t)
...
func Run(ctx context.Context,
	...
	defer func() {
		if r := recover(); r != nil {
			...
			if !ok && s!=apishared.ControlledExit {
				slog.Error("Run Greeting: trapped a panic in the guest side", "recovered", r)
				debug.PrintStack()
			}
		}
	}()
	...
	for {
		...
		if kerr == syscall.KernelErr_ReadOneTimeout {
			...
			slog.Info("calling backgrounder of Greeting")
			bg.Background(ctx)
			continue
		}
...
func ReadOneAndCall(ctx context.Context, binding *lib.ServiceMethodMap, 
	...
	if fn==nil {
		slog.Error("Greeting, readOneAndCall:unable to find binding for method on service, ignoring","mid",mid.Short(),"sid", sid.Short(),
...
func bind(ctx context.Context,sid id.ServiceId, impl Greeting) (*lib.ServiceMethodMap, syscall.KernelErr) {
...
//
// greeting.v1.Greeting.FetchGreeting
//
	...
	smmap.AddServiceMethod(sid,mid,"Greeting","FetchGreeting",
		GenerateFetchGreetingInvoker(impl)) 
	return smmap,syscall.KernelErr_NoError
}
 

// Locate finds a reference to the client interface of Greeting.  
func Locate(ctx context.Context,sid id.ServiceId) (Client,syscall.KernelErr) {
    cs, kerr:=client.LocateDynamic(ctx, "greeting.v1","greeting", sid)
    if kerr!=syscall.KernelErr_NoError{
        return nil, kerr
    }
    return &Client_{
...
func MustLocate(ctx context.Context, sid id.ServiceId) Client {
    ...
    normal:="unable to locate greeting.v1.greeting:"+name
    if err!=0 {
...
func Register() (id.ServiceId, syscall.KernelErr){
	...
	debugName:=fmt.Sprintf("%s.%s","greeting.v1","greeting")
...
func MustRegister() (context.Context,id.ServiceId) {
    sid, err:=Register()
    if err!=syscall.KernelErr_NoError {
        slog.Error("unable to register","package","greeting.v1","service name","greeting")
        panic("unable to register "+"greeting")
    }
...
func MustRequire(ctx context.Context, sid id.ServiceId) {
    ...
    if err!=syscall.KernelErr_NoError {
        if err==syscall.KernelErr_DependencyCycle{
            slog.Error("unable to require because it creates a dependcy loop","package","greeting.v1","service name","greeting","error",syscall.KernelErr_name[int32(err)])
            panic("require greeting.v1.greeting creates a dependency loop")
        }
        slog.Error("unable to require","package","greeting.v1","service name","greeting","error",syscall.KernelErr_name[int32(err)])
        panic("not able to require greeting.v1.greeting:"+syscall.KernelErr_name[int32(err)])
    }
}
...
func MustExport(ctx context.Context, sid id.ServiceId) {
    ...
    if err!=syscall.KernelErr_NoError{
        slog.Error("unable to export","package","greeting.v1","service name","greeting")
        panic("not able to export greeting.v1.greeting:"+syscall.KernelErr_name[int32(err)])
    }
}


func LaunchService(ctx context.Context, sid id.ServiceId, impl  Greeting) (*lib.ServiceMethodMap,*syscallguest.LaunchFuture,syscall.KernelErr) {
...
func MustLaunchService(ctx context.Context, sid id.ServiceId, impl Greeting) (*lib.ServiceMethodMap, *syscallguest.LaunchFuture) {
...
func GenerateFetchGreetingInvoker(impl Greeting) future.Invoker {
	return &invokeFetchGreeting{fn:impl.FetchGreeting} 
}  
