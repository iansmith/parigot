...
func Launch(ctx context.Context, sid id.ServiceId, impl Greeting) *future.Base[bool] {
	...
	ready.Handle(func (b bool) {
		...
		guest.Log(ctx).Error("Unable to start greeting.v1.Greeting, Ready returned false")
...
func Init(require []lib.MustRequireFunc, impl Greeting) (*lib.ServiceMethodMap,*syscallguest.LaunchFuture, context.Context, id.ServiceId){
	defer func() {
		if r := recover(); r != nil {
			guest.Log(context.Background()).Info("InitGreeting: trapped a panic in the guest side","recovered", r)
		}
	}()
	...
	launchF.Failure(func (err syscall.KernelErr) {
		...
		guest.Log(ctx).Error("launch failure on call Greeting","error",t)
...
func Run(ctx context.Context,
	...
	for {
		...
		if kerr == syscall.KernelErr_ReadOneTimeout {
			...
			guest.Log(ctx).Info("calling backgrounder of Greeting")
			bg.Background(ctx)
			continue
		}
...
func MustRegister() (context.Context,id.ServiceId) {
    sid, err:=Register()
    if err!=syscall.KernelErr_NoError {
        guest.Log(context.Background()).Error("unable to register","package","greeting.v1","service name","greeting")
...
func MustRequire(ctx context.Context, sid id.ServiceId) {
    ...
    if err!=syscall.KernelErr_NoError {
        ...
        guest.Log(ctx).Error("unable to require","package","greeting.v1","service name","greeting","error",syscall.KernelErr_name[int32(err)])
