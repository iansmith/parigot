...
type SearchResult struct {
	...
	ProjectsFound map[string]*ProjectDecl // converts path to project marker to ProjectDecl
}
...
type TomlConfig struct {
	Parigot map[string]ServiceDecl
}
...
func searchForProjects(path string) *SearchResult {
	...
	result := &SearchResult{
		Root:          path,
		ProjectsFound: make(map[string]*ProjectDecl),
	}
	...
	for _, marker := range projectMarkerFiles {
		project := &ProjectDecl{
			...
			ServicesFound: make(map[string]*ServiceDecl),
		}
...
func parseTomlFile(tomlPath string, project *ProjectDecl) {
