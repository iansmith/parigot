...
func Launch(ctx context.Context, sid id.ServiceId, impl MethodCallSuite) *future.Base[bool] {
	...
	ready.Handle(func (b bool) {
		...
		slog.Error("Unable to start methodcall.v1.MethodCallSuite, Ready returned false")
...
func MustRegister() (context.Context,id.ServiceId) {
    sid, err:=Register()
    if err!=syscall.KernelErr_NoError {
        slog.Error("unable to register","package","methodcall.v1","service name","method_call_suite")
...
func MustRequire(ctx context.Context, sid id.ServiceId) {
    ...
    if err!=syscall.KernelErr_NoError {
        if err==syscall.KernelErr_DependencyCycle{
            slog.Error("unable to require because it creates a dependcy loop","package","methodcall.v1","service name","method_call_suite","error",syscall.KernelErr_name[int32(err)])
            panic("require methodcall.v1.method_call_suite creates a dependency loop")
        }
        slog.Error("unable to require","package","methodcall.v1","service name","method_call_suite","error",syscall.KernelErr_name[int32(err)])
...
func MustExport(ctx context.Context, sid id.ServiceId) {
    ...
    if err!=syscall.KernelErr_NoError{
        slog.Error("unable to export","package","methodcall.v1","service name","method_call_suite")
