...
func Launch(ctx context.Context, sid id.ServiceId, impl MethodCallSuite) *future.Base[bool] {
	...
	ready.Handle(func (b bool) {
		...
		pcontext.Errorf(ctx,"Unable to start methodcall.v1.MethodCallSuite, Ready returned false")
		pcontext.Dump(ctx)
...
func Init(ctx context.Context,require []lib.MustRequireFunc, impl MethodCallSuite) *lib.ServiceMethodMap{
	defer func() {
		if r := recover(); r != nil {
...
func MustRegister(ctx context.Context) id.ServiceId {
    ...
    if err!=syscall.KernelErr_NoError {
        pcontext.Fatalf(ctx,"unable to register %s.%s","methodcall.v1","method_call_suite")
...
func MustRequire(ctx context.Context, sid id.ServiceId) {
    ...
    if err!=syscall.KernelErr_NoError {
        if err==syscall.KernelErr_DependencyCycle{
            pcontext.Errorf(ctx,"unable to require %s.%s because it creates a dependcy loop: %s","methodcall.v1","method_call_suite",syscall.KernelErr_name[int32(err)])
            panic("require methodcall.v1.method_call_suite creates a dependency loop")
        }
        pcontext.Errorf(ctx,"unable to require %s.%s:%s","methodcall.v1","method_call_suite",syscall.KernelErr_name[int32(err)])
...
func WaitSatisfied(ctx context.Context, sid id.ServiceId, impl MethodCallSuite) (*lib.ServiceMethodMap,syscall.KernelErr) {
...
func MustWaitSatisfied(ctx context.Context, sid id.ServiceId, impl MethodCallSuite) *lib.ServiceMethodMap {
