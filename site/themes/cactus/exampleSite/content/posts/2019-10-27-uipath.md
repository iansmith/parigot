...
ウェブアプリとブラウザーでレコーディングを行うためのものです。コンテナーを生成し、既定で 入力をシミュレート (Simulate TypeFromProto)/クリック (Click) の入力メソッドを使用します。
...
* ウィンドウメッセージ(Windows Message)
    - バックグラウンド処理
    - キーボード対応
* シミュレート(Simulate TypeFromProto/Click)
    - 互換性が低い
    - バックグラウンド処理
    - フィールド内自動削除
